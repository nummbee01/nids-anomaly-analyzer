...
from scapy.all import DNS, ICMP, IP, TCP, UDP, conf, sniff
...
def analyze_threat(packet_info):
    ...
    if packet_info["protocol"] == "TCP" and "S" in packet_info["flags"]:
        threat_level = "High"
...
def packet_callback(packet):
    ...
    try:
        ...
        packet_info["action"] = (
            "Blocked" if threat_level in ["Critical", "High"] else "Allowed"
        )

        save_packet_log(packet_info)
...
def load_logs():
    """Load packet and alert logs"""
    packets = []
    alerts = []

    if os.path.exists("packet_log.json"):
        try:
...
# Header
st.title("🛡️ Advanced NIDS - Real-Time Network Monitoring")
...
with st.sidebar:
    ...
    interface = st.text_input(
        "Network Interface", value="", help="Leave empty for default (eth0, wlan0, en0)"
    )
    ...
    filter_option = st.selectbox(
        "🔍 Filter Threats", ["All", "Critical", "High", "Medium", "Low"]
    )
    ...
    st.metric("Rate", f"{st.session_state.packet_count_current} pkt/s")
...
with tab1:
    if st.session_state.packets:
        ...
        st.info("No packets captured. Start monitoring to see live data.")
    ...
    if st.session_state.alerts:
        for alert in st.session_state.alerts[:50]:
            if "Critical" in alert:
                st.markdown(
                    f"<div class='critical'>🚨 {alert}</div>", unsafe_allow_html=True
                )
            elif "High" in alert:
                st.markdown(
                    f"<div class='high'>⚠️ {alert}</div>", unsafe_allow_html=True
                )
            else:
                st.markdown(
                    f"<div class='medium'>⚡ {alert}</div>", unsafe_allow_html=True
                )
    else:
...
with tab3:
    ...
    **macOS:**
    ...
    - **High**: Port scans, DNS anomalies
    - **Medium**: Suspicious ports (21, 23, 135, 139, 445, 3389), large packets, ICMP
    - **Low**: Normal traffic
    ...
    ### 📂 Log Files
    - `packet_log.json` - All captured packets
    - `alert_log.txt` - Security alerts
    """)
